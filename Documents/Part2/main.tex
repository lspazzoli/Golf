...
\begin{document}
	...
	\section{Software Architecture Documentation}
		\subsection{Architecture requirements}
			\paragraph\indent
			This section will extract the architectural requirements from the software requirements.
				...
				\subsubsection{Quality requirements}
					...
					In order of importance our quality requirements are:
						\begin{list}{$\bullet$}{\leftmargin=1.5cm \itemindent=0em}
								\item \textbf{Reliability:} This is highly important as this might be the only place research could be backed up to, therefore, the system needs to be reliable. Reliability in this system is measured against the effectiveness of the backup measures in place as well as the system be available on demand.  
								\item \textbf{Security:} The system needs to be secure from anyone who does not (or should not) have permission to use.
								\item \textbf{Audibility:} This is measured in terms of is every change noted and logged somewhere.
								\item \textbf{Scalability:} There will not be a huge amount of users making use of the system but it is important that if need be every user can make use of the system concurrently.
								\item \textbf{Maintainability:} Measured against how effectively the system choses to relocate data due to age.
						\end{list}
				\subsubsection{Integration and access channel requirements}
					\paragraph\indent
						The system should be accessible to registered users easily through a web interface as well as a mobile (android) application. In terms of integration; the android application will need to be able to communicate with server so as to retrieve and update files on the server. The protocols that will be used will be HTTPS to send data to the server securely to increase this, the data will be sent over an SSL. On the back-end there will be a database storing all the users and papers so the Web and Mobile interfaces will need to be able to query those databases so a query language such as SQL will have to be implemented. 
				\subsubsection{Architectural constraints}
					\paragraph\indent
						Currently we have no architectural constraints.
